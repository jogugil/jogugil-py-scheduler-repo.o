#######################
## NOTA:
###  - Este script sólo funciona en Linux. Varias funciones convierten timestamps ISO con date -d "$timestamp" +%s
###  - get_pod_event_timeline usa jq. Hay que asegurarse que jq esté instalado en el entorno antes de ejecutar el script
###  - Comprobar que 'bc' esté instalado, necesario para cálculos de métricas compuestas, latencias y ratios de éxito.
#######################
#!/bin/bash
set -e
...
WORKER3_IMAGE="busybox"
POD_NAME="test-pod"
RESULTS_FILE="metrics/scheduler_metrics_$(date +%Y%m%d_%H%M%S).csv"

# Variables globales para métricas
declare -A METRICS_TEST_POD
declare -A METRICS_NGINX_POD
declare -A METRICS_WORKER3_POD
# Inicializar arrays
METRICS_TEST_POD=(
    ["latency"]="N/A"
...
calculate_composite_metrics() {
    ...
    local mem_numeric=$(echo "$res" | awk '{print $2}' | tr -d 'Mi')  
...
run_improved_latency_test() {
    ...
    t0_sec=$(date -u +%s.%N)  # Guardar segundos con decimales
    ...
    t1_ts=$(kubectl -n kube-system logs -l app=$SCHEDULER_NAME --timestamps 2>/dev/null | \
    ...
    if [[ -n "$t1_ts" ]]; then
        ...
        local t1_epoch=$(date -u -d "$t1_ts" +%s.%3N 2>/dev/null || echo "0")  # milisegundos
    ...
    # Si no se encontró, obtener de las trazas [LATENCY]
    if [[ "$scheduler_latency" == "N/A" && -n "$test_logs" ]]; then
        scheduler_latency=$(echo "$test_logs" | grep "\[LATENCY\] $NAMESPACE/$pod_name" | \
                        tail -1 | awk -F'=' '{print $2}' | sed 's/s//')
    fi
    echo "Latencia scheduler: $scheduler_latency segundos"
  ...
  # 6. CONTAR OPERACIONES LIST DESPUÉS del scheduling y CALCULAR DIFERENCIA
    ...
    if [[ -n "$scheduler_pod" ]]; then
        ...
        # Forzar que no sea negativo
        implicit_retries=$(( implicit_retries > 0 ? implicit_retries : 0 ))
    ...
    elif [[ "$pod_name" == "test-nginx-pod" ]]; then
    ...
    elif [[ "$pod_name" == "test-worker3-pod" ]]; then
        METRICS_WORKER3_POD["latency"]=$scheduler_latency
        METRICS_WORKER3_POD["latency_pending_running"]=$latency_pending_running
        METRICS_WORKER3_POD["list_ops"]=$list_ops
        METRICS_WORKER3_POD["cpu"]=$avg_cpu
        METRICS_WORKER3_POD["mem"]=$avg_mem
        METRICS_WORKER3_POD["pull_start_latency"]=$pull_start_latency
        METRICS_WORKER3_POD["retries"]=$retry_count
        METRICS_WORKER3_POD["implicit_retries"]=$implicit_retries
        METRICS_WORKER3_POD["events"]=$scheduler_events
    fi
...
wait_for_image() {
    IMAGE=$1
    NODES=$2   # ahora puede ser una lista de nodos separados por espacios
    TIMEOUT=${3:-60}
    INTERVAL=${4:-2}
...
# Carga el dockerfile del scheduler y la imagen dentro del clúster
limpiar_y_cargar_imagen_scheduler() {
    IMAGE_NAME=$1
    LOCAL=$2
    TIMEOUT=60
    INTERVAL=2
    ...
    # Obtenemos todos los nodos, pero solo cargaremos y esperaremos en control-plane
    CONTROL_PLANE_NODE="sched-lab-control-plane"
    NODES=$(kind get nodes --name "$CLUSTER_NAME")

    # Limpiar la imagen en todos los nodos
    for NODE in $NODES; do
    ...
    echo "=== Cargando imagen $IMAGE_NAME en Kind (solo control-plane) ==="
    kind load docker-image $IMAGE_NAME --name $CLUSTER_NAME --nodes $CONTROL_PLANE_NODE

    # Esperar solo en los nodos que nos interesan (control-plane)
    wait_for_image "$IMAGE_NAME" "$CONTROL_PLANE_NODE" "$TIMEOUT" "$INTERVAL"

    echo "=== Imagen $IMAGE_NAME disponible en control-plane ==="
}
...
main() {
    ...
    limpiar_y_cargar_imagen_scheduler "$SCHED_IMAGE" local "sched-lab-control-plane"
    
    # Desplegar scheduler
    echo "=== Desplegando scheduler ==="
    kubectl apply -f rbac-deploy.yaml
    kubectl rollout status deployment -n kube-system --timeout=120s
    kubectl get pods -n kube-system
    
    
    # CONFIGURANDO LABELS Y TAINTS EN NODOS
    ...
    # Test con nginx
    limpiar_y_cargar_imagen_scheduler "$NGINX_IMAGE" remoto "$ALL_NODES"
    echo "=== Lanzando test-nginx-pod ==="
    run_improved_latency_test "test-nginx-pod" "test-nginx-pod.yaml" "test_nginx"
    ...
    # Test con test-worker3-pod
    limpiar_y_cargar_imagen_scheduler "$WORKER3_IMAGE" remoto "$ALL_NODES"
    echo "=== Lanzando test-nginx-pod ==="
    run_improved_latency_test "test-worker3-pod" "test-worker3-pod.yaml" "test-worker3"

    # Análisis detallado usando las métricas ya calculadas
    analyze_scheduling_detailed "test-worker3-pod" "$NAMESPACE" "test-worker3-pod_detailed"
    ...
    printf "%-${col1}s | %-${col2}s | %-${col3}s | %-${col4}s | %-${col5}s | %-${col6}s | %-${col7}s | %-${col8}s | %-${col9}s | %-${col10}s\n" \
        "test-worker3-pod" \
        "${METRICS_WORKER3_POD["latency"]}" \
        "${METRICS_WORKER3_POD["latency_pending_running"]}" \
        "${METRICS_WORKER3_POD["list_ops"]}" \
        "${METRICS_WORKER3_POD["cpu"]}" \
        "${METRICS_WORKER3_POD["mem"]}" \
        "${METRICS_WORKER3_POD["pull_start_latency"]}" \
        "${METRICS_WORKER3_POD["retries"]}" \
        "${METRICS_WORKER3_POD["events"]}" \
        "${METRICS_WORKER3_POD["implicit_retries"]}"
